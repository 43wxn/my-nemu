...
static inline int csr_idx(word_t csr) {
  switch (csr) {
...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  uint32_t i = s->isa.inst;  // Modified: removed .val
...
#define INSTPAT_MATCH(s, name, type, ...) { \
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh , R, {
    ...
    int64_t t = a * b;
    R(rd) = (word_t)((uint64_t)t >> 32);
  });
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R, {
    uint64_t t = (uint64_t)src1 * (uint64_t)src2;
    R(rd) = (word_t)(t >> 32);
  });
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, I, {
    word_t csr_no = BITS(i, 31, 20);
    int idx = csr_idx(csr_no);
    word_t old = cpu.csr[idx];
    cpu.csr[idx] = src1;
    if (rd != 0) R(rd) = old;
  });
...
int isa_exec_once(Decode *s) {
  s->isa.inst = inst_fetch(&s->snpc, 4);  // Modified: removed .val
